...
void Renderer::Frame()
{
	...
	WVP = XMMatrixScaling(scale, scale, scale) * XMMatrixRotationRollPitchYaw(-1.57079632679f, 0.0f, 0.0f) * XMMatrixTranslation(0.0f, 0.0f, 0.0f) * this->mCamera->GetViewMatrix() * this->mCamera->GetProjectionMatrix();
	WVP = XMMatrixTranspose(WVP);
	DirectX::XMStoreFloat4x4(&vsConstData.mWorldViewProj, WVP);
	this->mDeviceContext->UpdateSubresource(
		this->mWVPBuffer,
