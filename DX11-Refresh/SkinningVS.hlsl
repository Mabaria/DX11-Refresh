...
VSOut SKIN_VS(SKIN_VSIn input)
{
	...
	{
		v		+= input.blendWeights[i] * mul(input.Pos, gBoneTransforms[input.blendIndices[i]]);
		norm	+= input.blendWeights[i] * mul(input.Normal, gBoneTransforms[input.blendIndices[i]]);
		lastWeight -= input.blendWeights[i];
	}
	// Apply last weight
	v += lastWeight * mul(input.Pos, gBoneTransforms[input.blendIndices[MAX_AFFECTING_BONES - 1]]);
	norm += lastWeight * mul(input.Pos, gBoneTransforms[input.blendIndices[MAX_AFFECTING_BONES - 1]]);
	v.w = 1.0f;
